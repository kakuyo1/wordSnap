...
- 2026-03-28：M4 脚本修正（非 TDD）：`package-win.ps1` 改为默认不跑测试且仅打包程序本体（可选资源需显式开启），并在打包完成后自动清理 `staging`；`publish-release.ps1` 移除 `gh release view` 预检查以兼容“release 尚不存在”时的 CLI 错误行为。
- 2026-03-28：M4 图标统一修正（非 TDD）：新增 `src/resources/wordSnap.qrc` 并将托盘/应用图标切换为 `wordSnapLogo.png` 资源；`package-win.ps1` 增加 PNG->ICO 生成并传递给 `wordSnapV1.iss`，统一安装器与快捷方式图标来源。验证方式：本地 `cmake --build build --config Debug --target wordSnapV1` 通过。
- 2026-03-28：M4 打包回归修正（非 TDD）：修复 `package-win.ps1` 在 Windows PowerShell 下 PNG 维度位移解析导致的图标生成失败；改为先将 `wordSnapLogo.png` 高质量缩放到 256x256 再写入 ICO，避免 Inno Setup `Resource update error: File is too large`。同时构建命令限定为 `--target wordSnapV1`，不再构建测试目标。验证方式：`package-win.ps1 -Version 1.0.0 -Clean` 端到端成功产出安装包与 SHA256。
- 2026-03-28：M4 文档收口同步（非 TDD）：将 `docs/PLAN.md` 中 M4 状态调整为已完成，并同步更新 `README.md` 路线图与 `docs/FUTURE.md` 候选口径（M4 改为增量优化候选）。评估结论：本轮为文档一致性维护，不涉及核心业务逻辑代码，采用文档审阅校验。
- 2026-03-28：设置页新增“开机自启”功能（部分 TDD）：评估结论为“设置持久化逻辑适合 TDD，Windows 注册表写入属于平台集成行为走手工验证”。新增 `SettingsServiceTest`（`unit`）覆盖 `launch_on_startup` 默认值与读写持久化；新增 `StartupLaunchManager` 在保存设置时同步更新当前用户 `Run` 启动项，并在初始化时按配置对齐。验证方式：`ctest --test-dir build -C Debug -R "^SettingsServiceTest$" --output-on-failure` + Windows 手工勾选/取消开机自启验证。
