...
- 新增 `TESTING_GUIDELINES.md` 作为测试强约束，并同步到 `AGENTS.md` / `TDD_CORE.md` / `TDD_PLAN.md` / `README.md` / `PLAN.md` 的测试执行口径。
...
- 按产品反馈恢复 AI loading 原始动效：撤回颜色+字符帧方案，恢复三点透明度切换动画，保持当前几何稳定策略不变。
- 调整 AI loading 三点动效：活动点采用“上浮 + 放大”呈现，提升跳跃感与可感知性，且不改变卡片几何稳定策略。
...
- 新增 Windows 打包脚本 `scripts/release/windows/package-win.ps1`：默认仅构建 `wordSnapV1` 目标并打包程序本体与 Qt 运行时（不执行测试、不拷贝可选资源），支持按需开启测试/附加资源，完成后自动清理 `staging` 临时目录；已修复 `wordSnapLogo.png` 转 `.ico` 的尺寸兼容问题。
...
- 设置页新增“开机自启（Launch on Windows startup）”开关：配置可持久化，保存后会同步更新 Windows 当前用户 `Run` 启动项。
