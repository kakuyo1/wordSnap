...
void AppController::onRegionSelected(const QRect& globalRect) {
    ...
    if (captured.isNull()) {
        ...
        resultCardWidget_->showMessage(QStringLiteral("wordSnap V1"), message, QCursor::pos(), 2200);
        trayController_->showInfo(
            QStringLiteral("wordSnap V1"),
            message,
            1400);
    ...
    if (!ocrResult.success) {
        ...
        QToolTip::showText(QCursor::pos(), message);
        resultCardWidget_->showMessage(QStringLiteral("wordSnap V1"), message, QCursor::pos(), 2600);
    ...
    if (!entry.found) {
        ...
        QToolTip::showText(QCursor::pos(), message);
        resultCardWidget_->showMessage(QStringLiteral("OCR"), ocrResult.normalizedText, QCursor::pos(), 2800);
    ...
    const QString preview = pickDefinitionPreview(entry, settings_.displayMode);
    const QString tooltipText = preview.isEmpty()
                                    ? headword
                                    : QStringLiteral("%1\n%2").arg(headword, preview);

    QToolTip::showText(QCursor::pos(), tooltipText);
    resultCardWidget_->showMessage(headword, preview, QCursor::pos(), 5000);
